...
**头部布局优化**
- 🎨 **统一头部布局**：今日天气和城市天气页面头部采用一致的布局结构，动画居上、温度居中、信息一行排开
- 🎨 **今日天气状态行精简**：状态指示器改为纯图标，去除冗余文字标签，视觉更清爽
- 🎨 **信息chip高度优化**：湿度/风力/气压/能见度标签高度从60px降至48px，更紧凑
- 🎨 **农历日期独立展示**：农历日期从天气文字行中分离，单独一行显示

**Bug 修复**
- 🐛 **暗色模式背景修复**：修复暗色模式下scaffoldBackgroundColor和colorScheme硬编码为蓝色主题色的问题，现在正确跟随用户选择的主题方案切换
- 🐛 **城市天气预警优化**：城市天气页面预警按钮从头部移至AppBar，头部区域更简洁
- 🧹 **代码清理**：删除未使用的_buildLunarAndSolarTerm、_buildAlertIcon、_getCacheAgeText等方法，减少编译警告
