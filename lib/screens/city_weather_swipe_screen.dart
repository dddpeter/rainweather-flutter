...
class CityWeatherSwipeScreen extends StatefulWidget {
  ...
  const CityWeatherSwipeScreen({super.key, required this.cityName, this.cityId});
...
class _CityWeatherSwipeScreenState extends CityWeatherScreenBase<CityWeatherSwipeScreen>
  ...
  int _currentPage = 0;
  final List<String> _pageTitles = ['当前天气', '24小时预报', '15日预报'];
          ...
          Expanded(
            child: Center(
              child: Column(
                ...
                children: [
                  Icon(Icons.error_outline, color: AppColors.textPrimary, size: 64),
                  ...
                  ElevatedButton(
                    onPressed: () => weatherProvider.getWeatherForCity(cityName, cityId: cityId),
                    child: const Text('重试'),
                  ),
                ],
              ),
          ...
          Expanded(
            child: Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Icon(Icons.cloud_off_rounded, color: AppColors.textPrimary, size: 64),
                  const SizedBox(height: 16),
                  Text(
                    '暂无天气数据',
                    style: TextStyle(
  ...
  Widget _buildPageIndicator() {
    ...
    // 使用与 AppBar 一致的颜色
    final iconColor = themeProvider.isLightTheme ? AppColors.primaryBlue : AppColors.accentBlue;
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: isSelected
                          ...
                          ? AppColors
                                .textSecondary // 亮色模式：使用次要文字色，提高对比度
  ...
  Widget _buildHourlyForecastPage(WeatherProvider weatherProvider) {
    ...
    return RefreshIndicator(
      onRefresh: () async {
        await weatherProvider.getWeatherForCity(cityName, forceRefreshAI: true);
      },
