...
class Forecast15dService {
  ...
  Future<WeatherModel?> get15DayForecast(String cityId) async {
    try {
      final response = await _dio.get(
        'https://www.weatherol.cn/api/home/getCurrAnd15dAnd24h?cityid=$cityId',
      );
  ...
  Future<WeatherModel?> get15DayForecastForLocation(
    LocationModel location,
  ) async {
    String cityId = _getCityIdFromLocation(location);
    print('15-day forecast - Location: ${location.district}, CityID: $cityId');

    if (cityId.isNotEmpty) {
      final result = await get15DayForecast(cityId);
      if (result == null) {
        print('15-day forecast - Failed to fetch data for cityId: $cityId');
      }
      return result;
    } else {
      print(
        '15-day forecast - Failed to find cityId for location: ${location.district}',
      );
    }
    return null;
  }
