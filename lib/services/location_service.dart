...
class LocationService {
  ...
  final EnhancedGeocodingService _enhancedGeocodingService = EnhancedGeocodingService.getInstance();
  final BaiduLocationService _baiduLocationService = BaiduLocationService.getInstance();
  final TencentLocationService _tencentLocationService = TencentLocationService.getInstance();
  ...
  Future<LocationPermissionResult> checkLocationPermission() async {
    try {
      ...
      AppLogger.e('检查定位权限错误', error: e, stackTrace: stackTrace, tag: 'LocationService');
  ...
  Future<LocationPermissionResult> requestLocationPermission() async {
    try {
      ...
      AppLogger.e('请求定位权限错误', tag: 'LocationService', error: e, stackTrace: stackTrace);
      return LocationPermissionResult.error;
    }
  }
  ...
  Future<LocationModel?> getCurrentLocation() async {
    ...
    try {
      ...
      try {
        tencentLocation = await _tencentLocationService.getCurrentLocation().timeout(
        ...
        AppLogger.e('腾讯定位失败', tag: 'LocationService', error: e, stackTrace: stackTrace);
      }
      ...
      AppLogger.e('定位服务错误', tag: 'LocationService', error: e, stackTrace: stackTrace);
  ...
  bool _isLocationUnknown(LocationModel location) {
    ...
    // 检查城市、区县、省份是否包含未知值
    bool cityUnknown = unknownValues.contains(location.city.toLowerCase()) || location.city.isEmpty;
    bool districtUnknown =
        unknownValues.contains(location.district.toLowerCase()) || location.district.isEmpty;
    bool provinceUnknown =
        unknownValues.contains(location.province.toLowerCase()) || location.province.isEmpty;
  ...
  Future<bool> requestLocationService() async {
    try {
      ...
      if (!serviceEnabled) {
        ...
        Logger.log('位置服务开启请求结果: $serviceEnabled');
      }
      return serviceEnabled;
    } catch (e) {
      Logger.log('请求开启位置服务失败: $e');
      return false;
    }
  }
  ...
  Future<bool> requestAppLocationSettings() async {
    try {
      ...
      if (permission == LocationPermission.deniedForever) {
        ...
        Logger.log('打开应用设置结果: $opened');
        return false;
      }

      return permission == LocationPermission.whileInUse || permission == LocationPermission.always;
    } catch (e) {
      Logger.log('请求应用定位设置失败: $e');
      return false;
    }
  }
  ...
  Future<LocationModel?> getLocationWithGuidance() async {
    try {
      ...
      Logger.log('设备定位能力: $capabilities');

      // Check location service
      if (!capabilities['serviceEnabled']) {
        Logger.log('位置服务未开启，尝试请求开启');
      ...
      if (location == null) {
        Logger.log('所有定位方式都失败，建议用户检查网络和权限设置');
  ...
  Future<Position?> getSimpleLocation() async {
    try {
      ...
      if (!ok) {
        permission = await Geolocator.requestPermission();
        ok = permission == LocationPermission.whileInUse || permission == LocationPermission.always;
      }
      if (!ok) throw '无定位权限';
  ...
  Future<Map<String, dynamic>> validateGpsLocation() async {
    ...
    try {
      // 1. 检查权限
      Logger.log('🔍 验证GPS定位 - 步骤1: 检查权限');
      LocationPermission permission = await Geolocator.checkPermission();
      bool hasPermission =
          permission == LocationPermission.always || permission == LocationPermission.whileInUse;
      result['permission_check'] = hasPermission;
      Logger.log('权限状态: $permission, 有效权限: $hasPermission');
      ...
      Logger.log('🔍 验证GPS定位 - 步骤2: 跳过位置服务检测，直接尝试定位');
      bool serviceEnabled = await Geolocator.isLocationServiceEnabled();
      result['service_enabled'] = serviceEnabled;
      Logger.log('位置服务检测结果: $serviceEnabled (可能不准确，将通过实际定位验证)');

      // 3. 多层级定位策略
      Logger.log('🔍 验证GPS定位 - 步骤3: 开始多层级定位尝试');
      LocationModel? finalLocation = await _tryMultipleLocationMethods();

      if (finalLocation != null) {
        result['final_location'] = finalLocation;
        result['location_method'] = finalLocation.isProxyDetected ? 'IP定位' : 'GPS定位';

        // 如果成功定位，更新位置服务状态
        result['service_enabled'] = true;
        Logger.log('✅ 定位成功，使用方式: ${result['location_method']}');
  ...
  Future<LocationModel?> _tryMultipleLocationMethods() async {
    // 方法1: 尝试高精度GPS定位
    Logger.log('📍 尝试方法1: 高精度GPS定位');
    try {
      // 先请求权限
      LocationPermission permission = await Geolocator.requestPermission();
      if (permission == LocationPermission.denied ||
          permission == LocationPermission.deniedForever) {
        Logger.log('❌ GPS定位权限被拒绝');
      ...
      Logger.log('✅ GPS定位成功: ${position.latitude}, ${position.longitude}');
      ...
      if (location == null) {
        Logger.log('🔄 增强地理编码失败，尝试备用方法...');
        location = await _geocodingService.reverseGeocode(position.latitude, position.longitude);
      }

      if (location == null) {
        Logger.log('🔄 备用地理编码失败，尝试最终备用方法...');
      ...
      if (location != null) {
        ...
        if (_isLocationUnknown(location)) {
          Logger.log('⚠️ GPS定位成功但位置信息为"未知"，尝试IP定位作为备用');
          // 继续执行IP定位逻辑
        } else {
          Logger.log('✅ GPS定位完整流程成功');
          return location;
        }
      } else {
        Logger.log('❌ GPS定位成功但地理编码失败');
      }
    } catch (e) {
      Logger.log('❌ 高精度GPS定位失败: $e');
    }

    // 方法1.5: 尝试中等精度GPS定位（备用）
    Logger.log('📍 尝试方法1.5: 中等精度GPS定位');
    try {
      Position position = await getCurrentPositionChinaOptimized(
      ...
      Logger.log('✅ 中等精度GPS定位成功: ${position.latitude}, ${position.longitude}');
      ...
      if (location != null) {
        // 检查GPS定位的位置信息是否为"未知"
        if (_isLocationUnknown(location)) {
          Logger.log('⚠️ 中等精度GPS定位成功但位置信息为"未知"，尝试IP定位作为备用');
          // 继续执行IP定位逻辑
        } else {
          Logger.log('✅ 中等精度GPS定位完整流程成功');
          return location;
        }
      } else {
        Logger.log('❌ 中等精度GPS定位成功但地理编码失败');
      }
    } catch (e) {
      Logger.log('❌ 中等精度GPS定位失败: $e');
      Logger.log('🔄 开始尝试IP定位...');
    }

    // 方法2: 尝试IP定位
    Logger.log('📍 尝试方法2: IP定位');
    Logger.log('📡 正在初始化IP定位服务...');
    try {
      final ipLocationService = IpLocationService.getInstance();
      Logger.log('📡 开始调用IP定位接口...');
      final location = await ipLocationService.getLocationByIp();
      Logger.log('📡 IP定位接口调用完成，结果: ${location != null ? '成功' : '失败'}');

      if (location != null) {
        Logger.log('✅ IP定位成功: ${location.district}');
  ...
  Future<LocationModel?> getCurrentLocationWithBaidu() async {
    try {
      Logger.log('📍 使用百度定位获取当前位置...');
      LocationModel? location = await _baiduLocationService.getCurrentLocation();
      if (location != null) {
        Logger.log('✅ 百度定位成功: ${location.district}');
        _cachedLocation = location;
        return location;
      } else {
        Logger.log('❌ 百度定位失败');
