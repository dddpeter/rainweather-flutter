  ...
  for (; gbk_ind < gbk_buf.length;) {
    ...
    if (ch > 0x80) {
      ...
      if (word >= gbk_first_code &&
        ...
        uni_ptr.add(unicodeTables[word_pos]);
      }
    } else {
      gbk_ind++;
      uni_ptr.add(ch);
...
List<int> mask = <int>[0x7f, 0x3f, 0x1f, 0x0f, 0x7];
...
List<int> unicode2utf8(List<int> wordArray) {
  ...
  for (int i = 0, c = wordArray.length; i < c; ++i) {
    ...
    if (word <= 0x7f) {
      ...
      list.add(0xc0 | ((word >> 6) & 0x1f));
      list.add(0x80 | (word & 0x3f));
    } else if (word >= 0x800 && word < 0xffff) {
      list.add(0xe0 | ((word >> 12) & 0x0f));
      list.add(0x80 | ((word >> 6) & 0x3f));
      list.add(0x80 | (word & 0x3f));
    } else {
      //-1
      list.add(-1);
