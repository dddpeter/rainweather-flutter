...
class _AIInterpretationWidgetState extends State<AIInterpretationWidget> {
  ...
  Future<void> _loadLunarInterpretation({bool forceRefresh = false}) async {
    ...
    try {
      final prompt = '''【黄历解读】${widget.selectedDate.month}月${widget.selectedDate.day}日（${widget.lunarInfo.lunarDate}）
宜：${widget.lunarInfo.goodThings.isEmpty ? '诸事不宜' : widget.lunarInfo.goodThings.join('、')}
忌：${widget.lunarInfo.badThings.isEmpty ? '百无禁忌' : widget.lunarInfo.badThings.join('、')}
${widget.lunarInfo.isHuangDaoDay ? '✨ 黄道吉日，诸事大吉' : ''}
${widget.lunarInfo.solarTerm != null ? '节气：${widget.lunarInfo.solarTerm}' : ''}

【要求】用现代口语解读，总字数120-180字：
- 今日运势（1句话）
- 行事建议（宜忌的现代解读，2句话）
- 特别提醒（如果有）
